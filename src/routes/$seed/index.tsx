...
import {
  ModifierSelect,
  type SelectModifier,
} from '~/components/ModifierSelect'
...
const PageContent = observer(function PageContent() {
  ...
  const processedCoeffs = applyGlobals(
    previewCollection.coeffs,
    previewCollection.globals,
  )
  ...
  const onChannelOrderChange = (
    newCoeffs: AppCollection['coeffs'],
...
function ModifierSelectWrapper({ className }: { className?: string }) {
  ...
  const handleGlobalChange = (modifierIndex: number, value: number) => {
    ...
    const newSeed = serializeCoeffs(
      newPreviewData.coeffs,
      newPreviewData.globals,
    )
  ...
  const handleRGBChannelChange = (
    ...
    const currentCoeffs = previewData
      ? previewData.coeffs
    ...
    >
      ...
      <div className="flex flex-col flex-1 relative -bottom-3 lg:-bottom-2">
        {mod !== 'global' ? (
          ...
          <div className="h-full flex flex-col justify-between">
              ...
              return (
                <div className="flex-1 flex flex-col justify-center">
                  <ModifierSlider
                    ...
                    max={max}
                    onValueChange={(value) =>
                      handleGlobalChange(modifierIndex, value)
                    }
            ...
            {(() => {
              ...
              return rgbChannels.map((channel, channelIndex) => {
                const channelValue =
                  processedCoeffs[modifierIndex][channelIndex]
          ...
          <div className="h-full flex flex-col justify-between">
            {MODIFIERS.filter((modifier) => modifier !== 'global').map(
              (modifier) => {
