  ...
  for (let channel = 0; channel < numChannels; channel++) {
    ...
    for (let testD = -0.5; testD <= 1.5; testD += 0.02) {
      ...
      for (let i = 0; i < numColors; i++) {
        ...
        const computed =
          a[channel] + b[channel] * Math.cos(TAU * (c[channel] * t + testD))
 ...
 */
export function twoColorCosineCoeffs(
  from: number[],
  to: number[],
): CosineCoeffs {
 ...
 */
export function computeColorAtPosition(
  coeffs: CosineCoeffs,
  t: number,
): number[] {
  ...
  for (let channel = 0; channel < offsets.length; channel++) {
    ...
    const value =
      offsets[channel] +
      amplitudes[channel] *
        Math.cos(TAU * (frequencies[channel] * t + phases[channel]))
    ...
    for (let iter = 0; iter < iterations; iter++) {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        for (let j = 0; j < numChannels; j++) {
          ...
          const errorPlus = calculateTotalError(
            coeffs,
            originalColors,
            positions,
          )
          ...
          coeffs[i][j] = originalValue - delta
          const errorMinus = calculateTotalError(
            coeffs,
            originalColors,
            positions,
          )
          ...
          if (i === 1 && j !== 3) {
            ...
            coeffs[i][j] =
              Math.sign(coeffs[i][j]) * Math.max(0.1, Math.abs(coeffs[i][j]))
          }
        }
      }
...
export function generatePresetVariations(): CosineCoeffs {
  ...
  const a = preset.a.map((v) =>
    Math.max(0, Math.min(1, v * (0.9 + Math.random() * 0.2))),
  )
  const b = preset.b.map((v) =>
    Math.max(0, Math.min(1, v * (0.9 + Math.random() * 0.2))),
  )
