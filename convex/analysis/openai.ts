...
import { TagAnalysis } from '../schema'
...
export const processAllItems = internalAction({
  ...
  handler: async (ctx, { startIndexKey }) => {
    ...
    const { page, indexKeys, hasMore } = await ctx.runQuery(
      internal.analysis.common.batchOfItems,
      {
        startIndexKey,
      },
    )
    ...
    const results = await Promise.all(
      page.map((item) =>
        ctx.runAction(internal.analysis.openai.analyzeColorsWithOpenAI, {
          seed: item.seed,
        }),
      ),
    )
    ...
    await Promise.all(
      page.map((collection, i) => {
        ...
        return ctx.runMutation(
          internal.analysis.common.insertCollectionsAnalysis,
          {
    ...
    if (hasMore) {
      ...
      await ctx.scheduler.runAfter(
        15000,
        internal.analysis.openai.processAllItems,
        {
