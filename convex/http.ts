...
http.route({
  ...
  handler: httpAction(async (ctx, request) => {
    ...
    try {
      ...
      // Extract the first two colors from the processed colors for the logo gradient
      const logoColor1 = `rgb(${Math.round(processedColors[0][0] * 255)}, ${Math.round(processedColors[0][1] * 255)}, ${Math.round(processedColors[0][2] * 255)})`;
      const logoColor2 = `rgb(${Math.round(processedColors[Math.min(1, processedColors.length - 1)][0] * 255)}, ${Math.round(processedColors[Math.min(1, processedColors.length - 1)][1] * 255)}, ${Math.round(processedColors[Math.min(1, processedColors.length - 1)][2] * 255)})`;
      
      const fullSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630">
        ${svgString.replace('<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630">', '').replace('</svg>', '')}
        <g transform="translate(400, 265)">
          <rect width="400" height="150" rx="16" fill="rgba(0, 0, 0, 0.7)" />
          <g transform="translate(90, 50) scale(1.2)">
            <defs>
              <linearGradient x1="0%" y1="0%" y2="0%" id="logoGradient">
                <stop stopColor="${logoColor1}" offset="0%" />
                <stop stopColor="${logoColor2}" offset="100%" />
              </linearGradient>
            </defs>
            <g fill="none" fillRule="evenodd">
              <path
                ...
                fill="white"
              />
              <path fill="url(#logoGradient)" d="M93 43h34v7H93z" />
            </g>
          </g>
        </g>
      </svg>`;
