...
export const toggleLikeSeed = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existingLike !== null) {
      ...
      await publicLikesBySeed.delete(ctx, existingLike);
    ...
    const isPublic = true; // Note: using 'public' instead of 'isPublic' to match your schema
    ...
    if (newLike && newLike.isPublic) {
      await publicLikesBySeed.insert(ctx, newLike);
    }
