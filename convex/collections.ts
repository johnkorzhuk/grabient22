      ...
      if (existingCollection) {
        ...
        try {
          ...
          try {
            ...
            console.error(
              `Failed to deserialize seed directly ${seed}:`,
              innerError,
            )
...
export const listCollections = query({
  args: {
    ...
    sort: v.optional(
      v.union(v.literal('popular'), v.literal('new'), v.literal('old')),
    ), // "popular", "new", "old"
  },
  handler: async (ctx, args) => {
    ...
    const collections = collectionsWithNulls.filter(
    ...
    const results = await Promise.all(
      args.seeds.map(async (seed) => {
        const like = await ctx.db
          .query('likes')
          .withIndex('byUserIdAndSeed', (q) =>
            q.eq('userId', args.userId!).eq('seed', seed),
          )
          .first()
...
export const checkUserLikedSeed = query({
  ...
  handler: async (ctx, args) => {
    ...
    const like = await ctx.db
      .query('likes')
      .withIndex('byUserIdAndSeed', (q) =>
        q.eq('userId', args.userId!).eq('seed', args.seed),
      )
      .first()
